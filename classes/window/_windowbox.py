...
class WindowChoices(WindowUser):
    ...
    def draw_buttons(self, texts):
        y = (self.lineheight-self.font.get_linesize())/2
        width, height = self.contents_size
        for text in texts:
            w = text.get_width()
            x = 0#(width - w)/2
    ...
    def set_index(self, event):
        if self.contents_rect.collidepoint(event.pos):
            y = event.pos[1]-self.rect.y - self.offset
            self.index = y // self.lineheight
            
    def loop(self):
        "return the index the selected choice, return None if canceled"
        
        screen = pygame.display.get_surface()
        self.selected_rect = Rect(self.offset, self.offset,
                                  self.contents_rect.width, self.lineheight)
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.event.post(event)
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE and self.cancel_allowed:
                     ...
                     event.button == 1:
                        y = event.pos[1]-self.rect.y-self.offset
                        choice = y // self.lineheight
                        return choice
                elif event.type == pygame.MOUSEMOTION and\
                     self.contents_rect.collidepoint(event.pos):
                    self.set_index(event)
            self.selected_rect.y = self.index*self.lineheight+self.offset
            self.draw(screen)
            pygame.display.flip()
            pygame.time.Clock().tick(30)
